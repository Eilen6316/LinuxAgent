...
> Claude Code 在本仓库工作时必读的入口文档。
> 与 `AGENTS.md` 的**公共章节**必须保持同步；以 `## Claude 专属` 或 `## Agent 专属提示` 开头的章节允许按 agent 分叉，修改时无需对齐（但需保留明确标题）。
...
| 编码约定 | `.work/rule/*.md` | 不要写进 plan、prompt、或 CLAUDE.md |
...
## Claude 专属：记忆系统与 .work/ 的关系

- `.work/` 是**项目共享**的持久文档，所有 agent 和人类都读它
- `~/.claude/projects/*/memory/` 是**Claude Code 私有**的跨会话记忆
- 当二者冲突时，`.work/` 优先 —— memory 可能已过时

如果 memory 里的条目与 `.work/` 当前内容矛盾，更新或删除 memory，不要用 memory 覆盖 `.work/` 的决策。
