...
LinuxAgent v4.1.0 适合受控的人机协同运维场景，不应被当作无人值守自动修复器。
...
- [ ] 集群主机使用低权限 SSH 账号，而不是 root。
- [ ] 远端 profile 设置了预期 `remote_cwd`，生产 profile 优先使用 `environment: clean`。
- [ ] 如需 sudo，必须使用 `sudo -n`，并配置最小命令 allowlist。
- [ ] audit log 路径位于持久本地存储。
- [ ] 事故复盘流程包含 `linuxagent audit verify`。
- [ ] 操作员知道 `--yes` 不会绕过命令级确认。
- [ ] 高影响工作流已完成文档化、演练，并在涉及 LinuxAgent 行为时覆盖 harness 场景。
...
- 先在小主机组验证，再扩大集群操作范围。
- 生产环境保持较低批量确认阈值。
- 除非 sudoers 条目和 `sudo_allowlist` 已一起 review，否则保持
  `remote_profile.allow_sudo: false`。
...
- 本地 OS sandbox 不保护 SSH 目标；远端保护依赖低权限账号设计、审计和确认。
