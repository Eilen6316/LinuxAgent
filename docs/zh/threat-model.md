...
| 本地子进程 | 以调用用户的权限执行；Plan 1 no-op 沙箱只记录 metadata |
...
| 操作员误以为沙箱隔离已启用 | 只有 no-op runner 时拒绝 `sandbox.enabled=true`；audit/telemetry 标记 `enforced=false` |

## 不在范围内

- 操作员批准后对任意命令做沙箱隔离。Plan 1 只记录 sandbox profile
  metadata；真实隔离 runner 属于后续工作。
- 防止恶意本地 root 用户修改文件。
