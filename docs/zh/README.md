<div align="center">
  ...
  <p>
    ...
    <a href="#开发"><img src="https://img.shields.io/badge/coverage-86.73%25-brightgreen?style=flat-square" alt="Coverage"></a>
...
| 可测试性 | 当前文档基线为 677 个单元测试通过、覆盖率 86.73%；另有 HITL YAML 场景、10 个集成冒烟测试、红队用例和可选 Anthropic 兼容验证 |
...
| 维度 | 旧实现 | 当前 `v4` |
|---|---|---|
| 单元测试 | 0 | **当前文档基线：677 通过；安装 Anthropic extra 后可运行兼容验证** |
| 覆盖率 | 0 | **86.73%**（`--cov-fail-under=80` 门禁；以当前 CI / 本地 `make test` 输出为准） |
...
6. 模型必须返回结构化 `FilePatchPlan`，包含 `request_intent`、目标文件、
   unified diff、风险说明、验证命令和可选权限变更。
7. 写入前会弹出 diff 确认面板，显示每个文件的 `+N / -M` 统计、紧凑代码片段、
   高风险路径、权限位变更和验证命令。
8. 小 diff 已完整显示时不会再次询问展开；大 diff 会分页，只在存在隐藏页时询问是否继续查看。
9. 多文件 patch 支持逐文件确认，用户可以只接受部分文件。
...
同一对话或 `/resume` 恢复这个对话后再问"列一下文件"时，`ls -la`
可直接执行，不再确认。`/new` 或切到另一个已保存会话不会继承这份权限。
...
make test      # pytest + 80% fail-under；当前文档基线见上方“测试与工程化”
