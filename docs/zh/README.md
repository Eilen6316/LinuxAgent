...
| 可测试性 | 默认 272 个单元测试 + 4 个可选 Anthropic 兼容测试 + 12 个 HITL YAML 场景 + 8 个集成冒烟测试，覆盖率 90%+ |
...
| 维度 | 旧实现 | 当前 `v4` |
|---|---|---|
| 单元测试 | 0 | **默认 272 通过；安装 Anthropic extra 后额外 4 个兼容测试通过** |
| 覆盖率 | 0 | **90.65%**（`--cov-fail-under=80` 门禁） |
