<div align="center">
  ...
  <p>
    ...
    <a href="#开发"><img src="https://img.shields.io/badge/coverage-89.72%25-brightgreen?style=flat-square" alt="Coverage"></a>
...
| Runbook | 内置 11 个 YAML Runbook，并在 LLM 命令生成前优先匹配；首步确认后可继续执行安全后续步骤 |
...
| 可测试性 | 默认 302 个单元测试 + 4 个可选 Anthropic 兼容测试 + 12 个 HITL YAML 场景 + 8 个集成冒烟测试，覆盖率 89%+ |
...
| 批量集群操作 | 静默扩散 | 目标数 ≥ `cluster.batch_confirm_threshold`（默认 2）强制 CONFIRM；SSH 前阻断 shell 语法 |
...
| 维度 | 旧实现 | 当前 `v4` |
|---|---|---|
| 单元测试 | 0 | **默认 302 通过；安装 Anthropic extra 后额外 4 个兼容测试通过** |
| 覆盖率 | 0 | **89.72%**（`--cov-fail-under=80` 门禁） |
...
| `ui` | `max_chat_history` | `20` | 最多保留的历史消息数；新会话不会自动加载 |
...
每次启动 CLI 都是一个新对话，默认不继承上次上下文。历史只能通过显式 slash 命令访问；输入 `/` 会打开命令补全菜单：

| Slash 命令 | 作用 |
|---|---|
| `/history` | 列出本机保存的历史；随后输入编号召回 |
