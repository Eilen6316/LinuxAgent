<div align="center">
  ...
  <p>
    ...
    <a href="#开发"><img src="https://img.shields.io/badge/coverage-90.67%25-brightgreen?style=flat-square" alt="Coverage"></a>
...
| Runbook | 内置 8 个 YAML Runbook，并在 LLM 命令生成前优先匹配；首步确认后可继续执行安全后续步骤 |
...
| 可测试性 | 默认 273 个单元测试 + 4 个可选 Anthropic 兼容测试 + 12 个 HITL YAML 场景 + 8 个集成冒烟测试，覆盖率 90%+ |
...
| 维度 | 旧实现 | 当前 `v4` |
|---|---|---|
| 单元测试 | 0 | **默认 273 通过；安装 Anthropic extra 后额外 4 个兼容测试通过** |
| 覆盖率 | 0 | **90.67%**（`--cov-fail-under=80` 门禁） |
