<div align="center">
  ...
  <p>
    ...
    <a href="#开发"><img src="https://img.shields.io/badge/coverage-89.64%25-brightgreen?style=flat-square" alt="Coverage"></a>
...
| 可测试性 | 默认 300 个单元测试 + 4 个可选 Anthropic 兼容测试 + 12 个 HITL YAML 场景 + 8 个集成冒烟测试，覆盖率 89%+ |
...
| 维度 | 旧实现 | 当前 `v4` |
|---|---|---|
| 单元测试 | 0 | **默认 300 通过；安装 Anthropic extra 后额外 4 个兼容测试通过** |
| 覆盖率 | 0 | **89.64%**（`--cov-fail-under=80` 门禁） |
...
每次启动 CLI 都是一个新对话，默认不继承上次上下文。历史只能通过显式 slash 命令访问：
