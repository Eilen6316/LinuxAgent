...
POLICY_CAPABILITY_MATRIX: tuple[PolicyOracleCase, ...] = (
    ...
    PolicyOracleCase(
        ...
        expected_level=SafetyLevel.BLOCK,
        capabilities=("filesystem.delete",),
        matched_rules=("PROTECTED_TREE_DELETE",),
        can_whitelist=False,
        rationale="protected system tree deletion must block",
    ),
    PolicyOracleCase(
        command="rm -rf /home/user/tmp",
        expected_level=SafetyLevel.CONFIRM,
        capabilities=("filesystem.delete", "filesystem.mutate"),
        matched_rules=("DESTRUCTIVE", "DESTRUCTIVE_ARG"),
        can_whitelist=False,
        rationale="ordinary recursive user tree deletion must not block",
    ),
    PolicyOracleCase(
    ...
    PolicyOracleCase(
        ...
        matched_rules=("BLOCK_DEVICE_MUTATE",),
        can_whitelist=False,
        rationale="protected block device format must block",
    ),
    PolicyOracleCase(
        command="dd of=/dev/mapper/vg-root if=/tmp/image",
        expected_level=SafetyLevel.BLOCK,
        capabilities=("block_device.mutate",),
        matched_rules=("BLOCK_DEVICE_MUTATE",),
        can_whitelist=False,
        rationale="protected LVM raw write must block",
    ),
    PolicyOracleCase(
        command="dd of=/tmp/img bs=1M count=1",
        expected_level=SafetyLevel.CONFIRM,
        capabilities=("block_device.mutate",),
        matched_rules=("DESTRUCTIVE",),
        can_whitelist=False,
        rationale="ordinary dd output file must not block",
    ),
    PolicyOracleCase(
