...
| 三级安全分类 | `SAFE` / `CONFIRM` / `BLOCK`，每级有独立 `matched_rule` 用于审计追溯 |
...
| 可测试性 | 179 个单元测试 + 10 个 HITL YAML 场景 + 集成测试骨架，覆盖率 85%+ |
...
| 审计记录 | 日志可选 | 每条 HITL 事件追加到 `~/.linuxagent/audit.log`，`0o600`，不轮转 |
...
|---|---|---|
| 单元测试 | 0 | **179 通过** |
| 覆盖率 | 0 | **85.65%**（`--cov-fail-under=80` 门禁） |
...
每次 HITL 事件以一行 JSON 追加到 `~/.linuxagent/audit.log`：
